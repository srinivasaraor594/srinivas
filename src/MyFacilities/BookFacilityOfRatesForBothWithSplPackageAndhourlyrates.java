...
public class BookFacilityOfRatesForBothWithSplPackageAndhourlyrates extends TestBase {
	...
	public void BookingFacility(String EnterFacilityNameToSearch, String FacilityName, String EnterFromTime,
			String EnterToTime, String EnterDescription)
			throws InterruptedException, IOException, HeadlessException, AWTException {
		Reporter.log(
				"ScriptName: Facilityof RatesForBOTH BOOK - Admin Login  - Book for 7 hours (9 AM to 4 PM) - Should show Rs.5000 + 3000 = 8000 (Spl Package + hourly rate)",
				true);
		Reporter.log("----------------------------------------------------------------------------", true);
		Reporter.log(" ", true);
		...
		try {
			...
			try {
				...
				Reporter.log("File Name : " + GlobalVariablesCalling.ScreenShotsFileName
						+ "Booking Not Allowed For BOTH Facility With Admin Login", true);
			}
		} catch (NoSuchElementException Exception) {
		...
		Reporter.log("---------------------------", true);
		Reporter.log("File Name : " + GlobalVariablesCalling.ScreenShotsFileName
				+ "Book Facility Of Rates For Both For 7hrs", true);
		Reporter.log(" ", true);
