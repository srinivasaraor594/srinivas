...
public class BookFreeFacilityWith30MintsWithAdminLogin extends TestBase {
	...
	public void BookingFacility(String EnterFacilityNameToSearch, String EnterFromTime, String EnterToTime,
		...
		Reporter.log("ScriptName:AdminLogin ---->Application--->Facility--> Book  for 30 min  (Eg: 9 AM to 9:30 AM)",
				true);
		Reporter.log("----------------------------------------------------------------------------", true);
		Reporter.log(" ", true);
				...
				if (PriceDetailsButton.equals("Free")) {
					if (BookingButton.equals("Book / Cancel")) {
						...
						helper.TakeScreenShotOfWindowPopUp(
								"PopupCameAfter saving details of Booking of Free Facility  For 30 mints slot");
						helper.DeepSleep();
						helper.ProcessAlert();
						helper.Sleep();
					}
		...
		Reporter.log("File Name : " + GlobalVariablesCalling.ScreenShotsFileName
				+ "Search For FacilityWithFreeAnd Allow For Booking", true);
		Reporter.log("File Name : " + GlobalVariablesCalling.ScreenShotsFileName
				+ "PopupCameAfter saving details of Booking of Free Facility  For 30 mints slot", true);
		Reporter.log(" ", true);
