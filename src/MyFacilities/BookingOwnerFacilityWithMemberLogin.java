...
public class BookingOwnerFacilityWithMemberLogin extends TestBase {
	...
	public void BookingFacility(String EnterFacilityNameToSearch, String EnterFromTime, String EnterToTime,
		...
		Reporter.log("ScriptName:Member Login (Owner) - Book for 1 hours (9:00 to 10:00 AM) - Charge should be 1000",
				true);
		Reporter.log("----------------------------------------------------------------------------", true);
		Reporter.log(" ", true);
				...
				if (PriceDetailsButton.equals("Show")) {
					if (BookingButton.equals("Book / Cancel")) {
						...
						helper.TakeScreenShotOfWindowPopUp(
								"PopupCameAfter saving details of Booking of OwnerFacility with Specified Timings With MemberLogin");
						helper.MaxSleep();
						try {
							helper.ProcessAlert();
							helper.Sleep();
		...
		Reporter.log("File Name : " + GlobalVariablesCalling.ScreenShotsFileName
				+ "Search For OwnerFacility With MemberLogin", true);
		Reporter.log(
